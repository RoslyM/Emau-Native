...
export default function ServiceView() {
    ...
    const historique =  useSelector(state => state.historiquesP);
    ...
    useEffect(() => {
      ...
      setData(produits);
      setLoading(false);
    }, [historique,produits, dataImpression]);
      ...
      const incrementCount = (id) => {
        setCounts(prevCounts => {
          ...
          if (existingCountIndex !== -1) {
            ...
            const correspondingVisibleProduct = visibleProduit.find(item => item.id === id);
            if (correspondingVisibleProduct && correspondingVisibleProduct.quantity > updatedCounts[existingCountIndex].count) {
              updatedCounts[existingCountIndex].count += 1;
            } else {
              alert("Il n'y a pas assez de produit en stock !");
            }
      ...
      let generatePdf = async () => {
        ...
        const user = await AsyncStorage.getItem('userToken');
            ...
            try {
                ...
                await AsyncStorage.setItem('produits', JSON.stringify(data.map(item => {
                  const correspondingImpressionItem = dataImpression.find(impItem => impItem?.id === item.id);
                  if (correspondingImpressionItem) {
                    return { ...item, quantity: item.quantity - correspondingImpressionItem.count };
                  }
                  return item;
                })));
  ...
  return (
    <View className="bg-[#fafafa] flex-1">
      ...
      <SafeAreaView className="flex-1">
                ...
                <Icon.ShoppingBag className="text-gray-800" width={30} height={30}/>
                <Text className="text-center">Facture Produit</Text>
                </View>   
            ...
            return (
                    <View key={item.id} className="bg-white mb-3 flex flex-row justify-between px-5 py-3 mx-3">
                        ...
                        <View className="w-20">
                             <Text className="pb-3 text-gray-800">Quantité</Text>
    ...
    <View className="flex flex-col justify-center items-center absolute right-80 top-44">
        ...
        <View style={styles.centeredView}>
          ...
          <View className="flex flex-row space-x-5 items-center mt-5">
                <TouchableOpacity activeOpacity={1} delayPressIn={0} delayPressOut={0}  className="py-2 border-2 border-[#dfdfdf]  rounded-lg w-full flex-1" onPress={() => setModalVisibleDelete(!modalVisibleDelete)}>
                    <Text className="text-md text-gray-800  font-[600] pb-1 text-center">Annuler</Text>
                </TouchableOpacity>
                <TouchableOpacity activeOpacity={1} delayPressIn={0} delayPressOut={0}  className="bg-[#00b292] py-2 space-x-2 rounded-lg w-full flex-1 flex-row justify-center items-center" onPress={generatePdf}>
                <Icon.Printer stroke='white' width={30} height={30}/>   
