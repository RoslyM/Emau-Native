...
import {initializeProduit, deleteProduit} from '../slice/redux'
...
const validationSchema = yup.object().shape({
  ...
  quantity: yup.number().typeError('La quantité doit être un nombre').positive('La quantité doit être positif').required('La quantité est requis'),
});
...
export default function ServiceView() {
    ...
    const handleEdit = async (objet) => {
      ...
      const handleProduit = async (values) => {

        try {
          const newItem = { id: Date.now(), name: values.name, prix: values.prix , quantity: values.quantity};
          ...
          if (editData !== '') {
           ...
           await AsyncStorage.setItem('produits', JSON.stringify(updatedData));
           initializeProduit()
      ...
      const handleDelete = async ()=>{
        try {
            ...
            await AsyncStorage.setItem('produits', JSON.stringify(updatedData));
            dispatch(deleteProduit(selectDelete.id))
  ...
  return (
    <View className="bg-[#fafafa] flex-1">
      ...
      <SafeAreaView className="flex-1">
                ...
                <Text className="  text-center">Mes Produits</Text>
                </View>   
        ...
        <View className="flex-row items-center flex-1 rounded-2xl">
          ...
          <TextInput  value={seachService}  onChangeText={(text) => setSeachService(text)} placeholder='Rechercher en fonction du nom, du prix et de la quantité'  className="ml-3 text-gray-800 flex-1" />
        </View>
      ...
      <View className="flex flex-row bg-white">
        ...
        <View className="flex-1 p-2 ">
          <Text className="text-center text-gray-800  font-[500]">Quantité</Text>
        </View>
        <View className="flex-1 p-2 ">
      ...
      <ScrollView
        ...
        <View className="flex-1 p-2 border-2 border-gray-200">
          <Text className="text-center  text-gray-800 ">{item.quantity}</Text>
        </View>
        <View className="flex-1 p-2 border-2 border-gray-200">
    ...
    <Text className="pt-4 text-gray-400">Aucune donnée disponible</Text>
      ...
      <Modal
        ...
        onRequestClose={() => {
          ...
          <Formik
      initialValues={{ name: edit ?name : '', prix: edit?prix:'', quantity: edit?quantity:'' }}
      validationSchema={validationSchema}
      onSubmit={(values) => handleProduit(values)}
    >
        ...
        <View style={styles.centeredView} >
          <View style={styles.modalView}>
           ...
           <View className="flex flex-row items-center justify-between">
                <Text className="font-[500]  text-gray-800 ">{edit?'Modifier un Produit':'Ajouter un Produit'}</Text>
                <Pressable
              ...
              <Text className="text-gray-800  font-[500]  py-2">Nom du produit</Text>
          ...
          <Text className=" py-2 text-gray-800  font-[600]">Prix du produit</Text>
          ...
          <Text className="py-2 text-gray-800  font-[600]">Quantité du produit</Text>
          <View className={`flex flex-row justify-between items-center mt-1 border-2 px-4 rounded-xl ${touched.quantity && errors.quantity ? 'border-red-500' : 'border-gray-200'}`}>
          <TextInput
            className="text-gray-800  font-[500] py-2 flex-1"
            placeholderTextColor="gray"
            keyboardType="numeric"
          ...
          {touched.quantity && errors.quantity && <Text className="pt-2" style={{ color: 'red' }}>{errors.quantity}</Text>}
      ...
      <Modal
        ...
        <View style={styles.centeredView} >
          ...
          <View className="flex flex-row items-center justify-between mb-3">
                 <Text className="font-[500]  text-gray-800 ">Voulez vous supprimer le produit " {selectDelete.name} " ?</Text>
                <Pressable
          ...
          <View className="flex flex-row space-x-5 items-center mt-3">
